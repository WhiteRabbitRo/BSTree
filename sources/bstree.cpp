...
void right_sym_print(BSTree::Node* branch, int space) { //симметричный правый обход для функции вывода дерева
    if (branch == nullptr)
        return;
...
void BSTree::Tree::print(traversal_order ord) { //вывод узлов тремя способами
    switch (ord) {
        case direct: direct_print(root); break;
        case symmetric: symmetric_print(root); break;
        case reverse: reverse_print(root); break;
    }
}

auto print_oper(std::ostream& stream, const BSTree::Node* node) -> std::ostream&{ //прямой обход для оператора <<
    if (node != nullptr) {
        stream << node->data << " ";
...
auto BSTree::operator<<(std::ostream& stream, const Tree& tree) -> std::ostream& { //оператор вывода в поток
    Node* out_ = tree.root;
    if (out_ == nullptr) {
        stream << "Дерево пусто\n";
        return stream;
...
bool BSTree::Tree::remove (int val){ //удаление узла
    ...
    if ((add->left==nullptr) && (add->right!=nullptr)) {
        if (add->data > before->data)
            before->right = add->right;
        if (add->data < before->data)
            before->left = add->right;
    }
    if ((add->left!=nullptr) && (add->right==nullptr)) {
        if (add->data > before->data)
            before->right = add->left;
        if (add->data < before->data)
            before->left = add->left;
    }
    if ((add->left!=nullptr) && (add->right!=nullptr)) {
        Node* del = add->right;
        Node* bef = add;
...
void menu(BSTree::Tree* tree, bool empty){ //меню
    ...
    switch (ch) {
        ...
        case 2: {
            ...
            switch(way) {
                case 'a': tree->print(BSTree::direct); break;
                case 'b': tree->print(BSTree::symmetric); break;
                case 'c': tree->print(BSTree::reverse); break;
                default: cout << "Введено некорректное значение"; break;
            }
            cout << endl;
        } break;
        case 3: {
